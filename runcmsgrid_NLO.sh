...
#
# BMmodified: do reweighting for the first and only time!!!!
#
if [ -e ./reweight_card.dat ]; then
    cp ./reweight_card.dat ./Cards/
    echo "reweight=ON" > runscript.dat
    echo "done" >> runscript.dat
...
#
# BMmodified: remove mgrwgt block
#
if [ -e ./process/reweight_card.dat ]; then
    sed -i '/<mgrwgt>/,/<\/mgrwgt>/d' cmsgrid_final.lhe
fi
#
#
#
