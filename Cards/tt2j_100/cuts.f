...
c
     ...
     &        abs(ipdg(i)).eq.16).and.(abs(ipdg(j)).eq.12.or.
     &        abs(ipdg(j)).eq.14.or.abs(ipdg(j)).eq.16))) then
              if (ptZ(p(0,i),p(0,j)).lt.ptl) then
                 passcuts_user=.false.
                 return
              endif
            endif
...
c find the charged leptons (also used in the photon isolation cuts below)
     ...
     &    (ipdg(i).eq.12 .or. ipdg(i).eq.14 .or. ipdg(i).eq.16)) then
            is_a_lm(i)=.true.
            nlepton=nlepton+1
         else
            is_a_lm(i)=.false.
...
c transverse momentum
            if (ptl.gt.0d0) then
               if (pt_04(p(0,i)).lt.ptl.and.nlepton.eq.1) then
                  passcuts_user=.false.
                  return
               endif
            endif
